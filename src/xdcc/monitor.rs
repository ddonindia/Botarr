        ...
        tokio::spawn(async move {
            // Register monitor with deduplication
            {
                ...
                if monitors.iter().any(|m| {
                    m.plugin == plugin_name && m.network == network_name && m.channel == channel
                }) {
                    tracing::info!(
                        "[{}] Already monitoring {} on {}. Skipping duplicate.",
                        plugin_name,
                        channel,
                        network_name
                    );
            ...
            let network_name_inner = network_name.clone();
            ...
            let handle = tokio::spawn(async move {
                ...
                let network_name = network_name_inner;
                let channel = channel_inner;
                let _plugin_name = plugin_name_inner;
            ...
            }); // Close inner loop handle

            // Store the handle using tasks_arc in the outer spawn
            let mut tasks_lock = tasks_arc.write().await;
            tasks_lock
                .entry(plugin_name_for_tasks)
                .or_insert_with(Vec::new)
                .push(handle);
        }); // Close outer spawn
    }

    pub async fn stop_monitors_for_plugin(&self, plugin_name: &str) {
        tracing::info!("Stopping all IRC monitors for plugin: {}", plugin_name);

        // Abort background tasks
        {
            let mut tasks_lock = self.tasks.write().await;
            if let Some(handles) = tasks_lock.remove(plugin_name) {
        ...
        // Remove from active monitors status
        {
            let mut monitors_lock = self.active_monitors.write().await;
            monitors_lock.retain(|m| m.plugin != plugin_name);
        }
    }
